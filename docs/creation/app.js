...
// ============================================================================
// 27. v8 · Scroll-reveal 卷轴展开动画 · IntersectionObserver
// ============================================================================
(function initScrollReveal () {
  ...
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        // Delay slightly so user perceives "卷轴展开" as deliberate, not instant
        setTimeout(() => e.target.classList.add('revealed'), 180);
...
// ============================================================================
// 28. v8 · 故宫风同心圆 reticle 点击 → 注释面板
// ============================================================================
(function initReticles () {
  const reticles = document.querySelectorAll('.reticle');
  const panel = document.getElementById('case-reticle-panel');
  if (!reticles.length || !panel) return;
  const titleEl = document.getElementById('case-rp-title');
  const vEl     = document.getElementById('case-rp-v');
  const aEl     = document.getElementById('case-rp-a');
  const noteEl  = document.getElementById('case-rp-note');
  ...
  reticles.forEach(r => {
    r.addEventListener('click', (e) => {
      ...
      titleEl.textContent = r.dataset.rpTitle || '画上标注';
      vEl.textContent     = r.dataset.rpV     || '+0.00';
      aEl.textContent     = r.dataset.rpA     || '+0.00';
      noteEl.textContent  = r.dataset.rpNote  || '';
  ...
  document.addEventListener('click', (e) => {
    if (!e.target.closest('.reticle') && !e.target.closest('.reticle-panel')) {
      panel.classList.remove('visible');
      activeReticle = null;
    }
  });
